...
router.get("/", async (req, res) => {
  try {
    ...
    const result = await Pool.request().query(
      "exec [QMES].[dbo].[MANAGE_ITEM_RECEIVE_READ_SP] '전체','','000101','990101'"
    );
...
router.post("/", async (req, res) => {
  try {
    ...
    const result = await Pool.request()
      ...
      .query(
        "exec [QMES].[dbo].[MANAGE_ITEM_RECEIVE_READ_SP] @key,@input,@startDate,@endDate"
      );
    res.send(JSON.stringify(result.recordset));
  } catch (err) {
    res.status(500);
  ...
  try {
    ...
    const result = await Pool.request()
      .input(
        "입고일시",
        sql.NVarChar,
        !req.body.data.입고일시 ? "" : req.body.data.입고일시
      )
      .input(
        "품목코드",
        sql.NVarChar,
  ...
  try {
    ...
    for (var i = 0; i < req.body.data.length; i++) {
      ...
      await Pool.request()
        ...
        .query(`exec [QMES].[dbo].[MANAGE_ITEM_RECEIVE_DEL_SP] @key`);
    }
    res.send("삭제완료");
  } catch (err) {
    // console.log(err);
